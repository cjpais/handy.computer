...
If shortcuts that include a regular key (like Option+Space) suddenly stop triggering Handy — while modifier-only shortcuts (like just `fn`) still work — another process is likely holding **macOS Secure Input**. This is the system mechanism that protects password fields: while it is active, macOS stops delivering key events to apps like Handy. Recent versions of Handy detect this, show a warning banner in settings (naming the process when possible), and automatically switch affected shortcuts to a fallback so they keep working.
